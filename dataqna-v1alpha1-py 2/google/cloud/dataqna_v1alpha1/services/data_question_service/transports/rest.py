...
class DataQuestionServiceRestInterceptor:
    ...
    * Stripping extraneous information from responses
    ...
    """
    def pre_ask_question(self, request: data_question_service.AskQuestionRequest, metadata: Sequence[Tuple[str, Union[str, bytes]]]) -> Tuple[data_question_service.AskQuestionRequest, Sequence[Tuple[str, Union[str, bytes]]]]:
    ...
    def post_ask_question(self, response: rest_streaming.ResponseIterator) -> rest_streaming.ResponseIterator:
        ...
        DEPRECATED. Please use the `post_ask_question_with_metadata`
        interceptor instead.

        Override in a subclass to read or manipulate the response
        after it is returned by the DataQuestionService server but before
        it is returned to user code. This `post_ask_question` interceptor runs
        before the `post_ask_question_with_metadata` interceptor.
        """
        return response

    def post_ask_question_with_metadata(self, response: rest_streaming.ResponseIterator, metadata: Sequence[Tuple[str, Union[str, bytes]]]) -> Tuple[rest_streaming.ResponseIterator, Sequence[Tuple[str, Union[str, bytes]]]]:
        """Post-rpc interceptor for ask_question

        Override in a subclass to read or manipulate the response or metadata after it
        is returned by the DataQuestionService server but before it is returned to user code.

        We recommend only using this `post_ask_question_with_metadata`
        interceptor in new development instead of the `post_ask_question` interceptor.
        When both interceptors are used, this `post_ask_question_with_metadata` interceptor runs after the
        `post_ask_question` interceptor. The (possibly modified) response returned by
        `post_ask_question` will be passed to
        `post_ask_question_with_metadata`.
        """
    ...
    class _AskQuestion(_BaseDataQuestionServiceRestTransport._BaseAskQuestion, DataQuestionServiceRestStub):
        ...
        def __call__(self,
            ...
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(logging.DEBUG):  # pragma: NO COVER
                ...
                _LOGGER.debug(
                    f"Sending request for google.cloud.dataqna_v1alpha1.DataQuestionServiceClient.AskQuestion",
                    extra = {
            ...
            resp, _ = self._interceptor.post_ask_question_with_metadata(resp, response_metadata)
            return resp
